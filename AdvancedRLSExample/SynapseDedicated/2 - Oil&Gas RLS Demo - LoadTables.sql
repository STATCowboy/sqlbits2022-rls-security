...
BEGIN
	...
	SET  @wellName = 'WELL #' + cast(@i as varchar);

	SET @div = (select [DIVISION] from [ASSET_HIERARCHY] where id = @assetid);
	SET @reg = (select [DIVISION] from [ASSET_HIERARCHY] where id = @assetid);							  
	SET @ag = (select [ASSET_GROUP] from [ASSET_HIERARCHY] where id = @assetid);
	SET @at = (select [ASSET_TEAM] from [ASSET_HIERARCHY] where id = @assetid);
...
BEGIN
  ...
  BEGIN
	...
	BEGIN
		...
		SET @oil = @oil + CAST(RAND() * 20 AS INT);
	    SET @gas = @gas + CAST(RAND() * 20 AS INT);
	    SET @ngl = @ngl + CAST(RAND() * 3 AS INT);
...
BEGIN
  ...
  BEGIN
    ...
    SET @rc = CAST(RAND() * 3 AS INT) + 1;
	SET @hr = CAST(RAND() * 24 AS INT) + 1;
